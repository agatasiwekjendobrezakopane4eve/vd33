#!/usr/bin/env python
# video_grid_full_pipeline33R.py
#
# 33R = 33Q but with "social preview" support:
#
#  - Thumbnails are embedded as data:image/...;base64,... URIs
#    (from Selenium screenshots and/or remote oEmbed thumbs)
#  - Screenshot PNG files are removed locally after embedding
#  - Only the AES/obf B HTML is added to git (A is for local use only)
#  - A real OG collage PNG is built from non-data-URI thumb sources
#  - The collage is referenced in:
#        * the plain A HTML (for local viewing/sharing)
#        * the AES wrapper B HTML (so Facebook/LinkedIn see it
#          even though the body is just the password box)
#
# Usage examples:
#   python video_grid_full_pipeline33R.py urls.csv 3
#   python video_grid_full_pipeline33R.py urls.csv 3 screenshot
#   python video_grid_full_pipeline33R.py urls.csv 3 screenshot ogmax=60 aes
#   python video_grid_full_pipeline33R.py urls.csv 3 aes
#
...
def wait_for_github_page(url: str, max_checks: int = 10, delay: int = 3) -> None:
    for i in range(1, max_checks + 1):
        try:
            r = requests.get(url, timeout=6)
            if r.ok:
                print("✅ GitHub Pages is now serving the new grid page.")
                return
        except Exception:
            pass
        print(f"🌐 GitHub check {i}: {url}")
        time.sleep(delay)
    print("⚠️ Gave up waiting for GitHub Pages (it may still appear shortly).")
...
def parse_args(argv: list[str]):
    if len(argv) < 2:
        print(
            "Usage: video_grid_full_pipeline33R.py "
    ...
    if engine != "selenium":
        print("❌ Only 'selenium' engine is supported in 33R.")
        sys.exit(1)
...
def main():
    ...
    commit_msg = f"Add new video grid page (multi-platform v33R vd3, data-URI thumbs, AES/obf, OG collage) ({ts})"
    ...
    print("\n✅ FULL PIPELINE COMPLETE (v33R selenium-only, data-URI thumbs, AES/obf options, OG collage).")
