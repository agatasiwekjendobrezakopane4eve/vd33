#!/usr/bin/env python
# video_grid_full_pipeline33S.py
#
# 33S = 33R but with:
#  - same behaviour (unique HTML + unique PNG per run)
#  - more robust git push (1 retry if it flakes)
#  - extra probing of BOTH the HTML page and the OG collage image
#    on GitHub Pages, to make social previews more reliable.
#
# Usage examples:
#   python video_grid_full_pipeline33S.py urls.csv 3
#   python video_grid_full_pipeline33S.py urls.csv 3 screenshot
#   python video_grid_full_pipeline33S.py urls.csv 3 screenshot ogmax=60 aes
#   python video_grid_full_pipeline33S.py urls.csv 3 aes
#
# NOTE: requires 'cryptography' and 'selenium', 'Pillow', 'requests':
...
from PIL import Image  # for OG collage (optional)
...
# vd3 paranoia: now we DO publish a separate OG collage by default
...
# ------------------ Selenium driver – EXACTLY like 8/9 ------------------ #
...
class VideoTile:
    ...
    thumb_src_collage: str | None  # original src (local path or URL) for OG collage
...
# ------------------ OG collage builder (uses thumb_src_collage) ------------------ #
...
def build_og_collage(tiles: list[VideoTile],
                     repo_root: Path,
                     max_cells: int = 63,
                     base_w: int = 1200,
                     base_h: int = 630) -> Path | None:
    """
    Build a collage PNG from up to max_cells thumbnails.
    OG image size fixed at base_w x base_h (good for FB/LinkedIn).

    Returns the path to the timestamped PNG (unique per run).
    """
    if max_cells <= 0:
        print("🧩 OG collage disabled (max_cells <= 0).")
        return None

    # Use the original sources (local PNGs or remote URLs), *not* data URIs.
    thumb_srcs: list[str] = []
    for t in tiles:
    ...
    if not thumb_srcs:
        print("🧩 No thumbnails available for OG collage.")
        return None
    ...
    for src in thumb_srcs:
        ...
        if im is not None:
            imgs.append(im)
    if not imgs:
        print("🧩 Failed to load any thumbs for OG collage.")
        return None
    ...
    print(f"🧩 Building OG collage with {n} thumbs "
    ...
    out_name = f"grid-thumb-collage-{ts}.png"
    out_path = repo_root / out_name
    collage.save(out_path, format="PNG")
    print(f"🖼  Collage OG image saved to: {out_path}")
    return out_path


# ------------------ HTML page builder ------------------ #

def build_page_html(tiles: list[VideoTile],
                    cols: int | None,
                    screenshot_mode: bool,
                    og_image_rel: str | None) -> str:
...
def make_aes_protected_wrapper(html: str,
    ...
    Facebook/LinkedIn/etc can show the collage even though the body
    is just the password box.
    """
...
def run_git_commands(files: list[Path], commit_msg: str) -> None:
    ...
    try:
        ...
        # carry on; maybe nothing changed
        pass

    # 33S: retry git push once if it flakes (e.g. HTTP 408)
    for attempt in (1, 2):
        try:
            print(f"💻 Running git: push (attempt {attempt})")
            subprocess.run(["git", "push"], check=True)
            return
        except Exception as e:
            print(f"⚠️ git push failed (attempt {attempt}): {e}")
            if attempt == 1:
                print("   Waiting 3s and retrying git push once more…")
                time.sleep(3)
            else:
                print("   Giving up on git push.")


def wait_for_github_page(url: str, max_checks: int = 15, delay: int = 3) -> None:
    for i in range(1, max_checks + 1):
        ...
        time.sleep(delay)
    print(f"⚠️ Gave up waiting for: {url} (it may still appear shortly).")
...
def parse_args(argv: list[str]):
    if len(argv) < 2:
        print(
            "Usage: video_grid_full_pipeline33S.py "
    ...
    og_max = 63  # default max tiles for OG collage (may be disabled)
    ...
    if engine != "selenium":
        print("❌ Only 'selenium' engine is supported in 33S.")
        sys.exit(1)
...
def main():
    ...
    print("🧩  OG collage tiles max:", og_max,
    ...
    # 4) Build tiles (convert thumbs to data URIs, preserve original src for collage)
    ...
    # 5) Build OG collage (before deleting screenshot files)
    collage_path = None
    og_image_rel = None
    if ENABLE_PUBLIC_OG_COLLAGE and og_max > 0:
        collage_path = build_og_collage(tiles, repo_root, max_cells=og_max)
        og_image_rel = collage_path.name if collage_path else None

    # 6) We no longer need local screenshot files once data URIs and collage are built
    if screenshot_mode and screenshot_rel_map:
        cleanup_screenshot_files(screenshot_rel_map, repo_root)

    # 7) Build full page HTML
    html = build_page_html(tiles, cols, screenshot_mode, og_image_rel)
    ...
    # 9) Git commit & push
    # vd3 paranoia: only push B (and optional collage), NOT A and NOT thumbs
    files_for_git: list[Path] = [html_b_path]
    if collage_path:
        files_for_git.append(collage_path)

    commit_msg = f"Add new video grid page (multi-platform v33S vd3, data-URI thumbs, AES/obf, OG collage) ({ts})"
    ...
    wait_for_github_page(public_url)

    # 33S extra: also probe the OG image URL if we have one
    if og_image_rel:
        og_url = f"{PUBLIC_BASE_URL}/{og_image_rel}"
        print("\n⏳ Probing GitHub Pages for the OG collage image…")
        wait_for_github_page(og_url)

    print("\n✅ FULL PIPELINE COMPLETE (v33S selenium-only, data-URI thumbs, AES/obf options, OG collage).")
    print("Files pushed to GitHub:")
    print(f"  • {html_b_path.name}")
    if collage_path:
        print(f"  • {collage_path.name}")
